...
def home():
    ...
    try:
        ...
        user_info = db.users.find_one({"username": payload["id"]})  # id를 꺼내와서 실제 유저 정보와 비교
        return render_template('index.html', user_info=user_info)  # 결과를 클라이언트에게 던져줌
...
def get_posts():
    ...
    try:
        ...
        # 포스팅 목록 받아오기
        # posts = list(db.posts.find({}).sort("date", -1).limit(20))  # 내림차순(가장 최근부터)으로 가져온다
        # 실제로 해당 포스트에 좋아요 눌렀는지, 어떤 포스트에 눌렀는지 구분하기 위한 고유식별자 필요: 이 디비에서는 _id
        for post in posts:
            post["_id"] = str(post["_id"])  # 각각의 아이디들을 문자열로 변환한다.
            # 좋아요 숫자 표시하기
...
def update_like():
    ...
    try:
        ...
        if action_receive == "like":
            db.likes.insert_one(doc)    # 좋아요를 한거라면 db에 넣어주고
        else:
            db.likes.delete_one(doc)    # 좋아요 취소했다면 db에서 빼줌
        # 현재글의 좋아요 개수를 계산해서 넘겨줌
